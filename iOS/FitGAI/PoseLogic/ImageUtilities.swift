...
class ImageUtilities: NSObject {
  ...
  class func image(from sampleBuffer: CMSampleBuffer?) -> UIImage? {
    guard let sampleBuffer = sampleBuffer else {
      HHLog.e(.poseDetect, .no0033, "Sample buffer is NULL.")
      return nil
    }
    guard let imageBuffer = CMSampleBufferGetImageBuffer(sampleBuffer) else {
      HHLog.e(.poseDetect, .no0034, "Invalid sample buffer.")
      return nil
    }
    ...
    guard let context = CGContext(data: baseAddress, width: width, height: height, bitsPerComponent: bitPerComponent, bytesPerRow: bytesPerRow, space: colorSpace, bitmapInfo: CGBitmapInfo.byteOrder32Little.rawValue | CGImageAlphaInfo.premultipliedFirst.rawValue) else {
      HHLog.e(.poseDetect, .no0035, "Failed to create CGContextRef")
      CVPixelBufferUnlockBaseAddress(imageBuffer, .readOnly)
      return nil
    }

    guard let cgImage = context.makeImage() else {
      HHLog.e(.poseDetect, .no0036, "Failed to create CGImage")
      CVPixelBufferUnlockBaseAddress(imageBuffer, .readOnly)
      return nil
    }
  ...
  class func croppedSampleBuffer(_ sampleBuffer: CMSampleBuffer,
    ...
    if error != kCVReturnSuccess {
      HHLog.e(.poseDetect, .no0037, "Crop CVPixelBufferCreateWithBytes error \(Int(error))")
      return nil
    }
    ...
    if error != kCVReturnSuccess {
      HHLog.e(.poseDetect, .no0038, "CMVideoFormatDescriptionCreateForImageBuffer error \(Int(error))")
    ...
    if error != kCVReturnSuccess {
      HHLog.e(.core, .no0039, "CMSampleBufferCreateForImageBuffer error \(Int(error))")
    }
    CVPixelBufferUnlockBaseAddress(imageBuffer, .readOnly)
    return resultBuffer
  }
